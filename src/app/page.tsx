...
import { getTopPlayersByPoints, getTopPlayersByBullseyes, getTopPlayersByTriples } from "@/lib/db/queries";

export default async function HomePage() {
  const [topPoints, topBullseyes, topTriples] = await Promise.all([
              ...
              <div className="space-y-3">
                {topPoints.map((player, index) => (
                  ...
                  >
                    ...
                    <span className="font-bold text-ldl-red">{player.totalPoints || 0} pts</span>
                  </div>
                ))}
              </div>
            </CardContent>
